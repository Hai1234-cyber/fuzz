import sys
sys.path.append('/home/ubuntu/attack/')
...
class Params:
    def __init__(self):
        ...
        self.influence_radius = 0.20 # [m] potential fields radius, defining repulsive area size near the obstacle
...
class Metrics:
    def __init__(self):
        ...
        self.folder_to_save = '/home/ubuntu/Desktop/'
count = [0, 0, 0, 0] #成功次数
class attack():
    def __init__(self, attack_strategy=1) -> None:
        ...
        # 'data_collection/attack_res/recorded_data.csv'
        ...
        self.count_success_data_to_save_file_path = self.attack_floder_path + 'recorded_data.csv' # 实验数据记录
        # 将地图转换成矩阵
        self.nrows = 500 # 矩阵行数
        self.ncols = 500 # 矩阵列表
        ...
        self.obstacles = [
            ...
            np.array([[-0.7, -1.8], [-0.5, -1.8], [-0.5, 0.5], [-0.7, 0.5]]),
    ...
    def crash_uav_check(self) -> bool:
        ...
        for i in range(len(self.robots)):
            for j in range(i+1, len(self.robots)):
                if norm(self.robots[i].sp - self.robots[j].sp) < 0.1:
                    return True
        return False
    ...
    def visualize2D(self, traj_global, sp_ind, P):
        ...
        plt.plot(x, y, marker=smiley, color='red', markersize=1, zorder=5)
...
def record_data(csv_filename):
    global count
    while True:
    # 在这里添加你要记录的数据获取逻辑
    # 示例：获取当前时间戳和一些随机值
        timestamp = time.time()
        value = count  # 这里的数据获取逻辑需要根据实际情况修改

        # # 记录数据
        #     Sdata = {'Timestamp': [], 'Value': []}
        #     Sdata['Timestamp'].append(timestamp)
        #     Sdata['Value'].append(value)
            
            # 存储路径
        # csv_filename = 'data_collection/attack_res/recorded_data.csv'
            # 创建一个包含当前数据的数据框
        ...
        print(f"记录数据：Timestamp={timestamp}, Value={value}")
            
            # 等待半小时
        time.sleep(3 * 60)  # 30分钟
        #     with open(self.count_success_data_to_save_file_path, 'a') as f:
        #         f.write(str(self.attack_point))  
def test():
    csv_filename = 'data_collection/attack_res/test2.csv'
    # record_thread = threading.Thread(target=record_data, args=(csv_filename,))
    # record_thread.start()
    # run_time_seconds = 6 * 60 * 60  # 6小时
    # run_time_seconds = 7 * 60
    # for _ in range(1):
    ...
    try:
        for iteration in range(20):
            value = count
            df = pd.DataFrame({'times': [iteration], 'Value': [value]})

            # 以追加模式写入CSV文件
            df.to_csv(csv_filename, mode='a', header=False, index=False)

            print(f"记录数据：times={iteration}, Value={value}")
        # while True:
            # elapsed_time = time.time() - start_time
            # if elapsed_time >= run_time_seconds:
            #     print("程序运行了6个小时，现在停止。")
            #     break
            attack_demo = attack(attack_strategy=1)
            attack_demo.start()
            time.sleep(1)
    except KeyboardInterrupt:
        # 用户按下Ctrl+C时，停止记录数据的线程
        # record_thread.join()
        print("用户中断，程序停止。")
